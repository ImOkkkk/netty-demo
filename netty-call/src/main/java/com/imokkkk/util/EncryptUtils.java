...
import com.imokkkk.kryo.KryoSerializer;
...
public class EncryptUtils {
    ...
    /** 对字符串进行MD5加盐摘要 先将str进行一次MD5摘要，摘要后再取摘要后的字符串的第1、3、5个字符追加到摘要串， 再拿这个摘要串再次进行摘要 */
    public static String encrypt(String str) {
        String encryptStr = EncryptByMD5(str);
        if (encryptStr != null) {
            encryptStr =
                    encryptStr + encryptStr.charAt(0) + encryptStr.charAt(2) + encryptStr.charAt(4);
            encryptStr = EncryptByMD5(encryptStr);
        }
        return encryptStr;
    }

    /** 对对象进行MD5摘要，先对对象进行序列化，转为byte数组， 再将byte数组转为字符串，然后进行MD5加盐摘要 */
    public static String encryptObj(Object o) {
        return encrypt(bytes2Hex(KryoSerializer.serialize(o)));
    }
}
